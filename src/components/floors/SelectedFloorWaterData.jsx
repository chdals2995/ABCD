// src/components/floors/SelectedFloorWaterData.jsx
...
export default function SelectedFloorWaterData({ floor }) {
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((child) => {
          ...
          const value = v.waterAvg ?? v.waterSum ?? v.water ?? 0;
      ...
      (err) => {
        console.error("SelectedFloorWaterData onValue error:", err);
        if (!isMounted) return;
  ...
  const options = {
    ...
    scales: {
      ...
      y: {
        ...
        max: yMax,
        beginAtZero: false,
        title: { display: true, text: "수도 사용량 (㎥)" },
      },
    },
  };
  ...
  return (
    <div className="w-full border border-gray-200 rounded-[10px] bg-white px-4 py-3 h-[220px]">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold">{floor} 수도 사용 (실시간)</h2>
        {loading && (

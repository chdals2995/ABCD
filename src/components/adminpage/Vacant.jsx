...
export default function Vacant() {
   ...
   useEffect(() => {
      ...
      const unsubs = lots.map((lot) => {
         ...
         return onValue(
            ...
            (snap) => {
               const raw = snap.val() || {};
               // 새 구조: { meta, slots }, 옛 구조: 슬롯들이 바로 밑에 있는 형태 둘 다 지원
            ...
            (error) => {
               console.error(
                  "Vacant parkingRealtime read error:",
                  lot.lotId,
                  error
               );
               setStats((prev) => ({
   ...
   return (
      <div className="w-[553px] h-[302px] border-[12px] border-[#054E76] rounded-[10px] p-[22px] bg-white flex flex-col">
         <h1 className="font-bold font-pyeojin text-[25px] mb-6">
            주차 잔여수
         </h1>
